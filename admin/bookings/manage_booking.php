...
<div class="container-fluid">
    <form action="" id="book-form">
        ...
        <div class="form-group">
            <label for="date_start" class="control-label"><i class="fas fa-calendar-day"></i> Дата получения аренды: </label>
        ...
        <div class="form-group">
            <label for="date_end" class="control-label"><i class="fas fa-calendar-check"></i> Дата возврата арендной платы: </label>
        ...
        <div class="form-group">
            <label for="rent_days" class="control-label"><i class="fas fa-calendar-check"></i> Дней/с аренды: </label>
        ...
        <div class="form-group">
            <label for="daily_rate" class="control-label"><i class="fas fa-calendar-day"></i> Дневная норма мотоцыкла: </label>
        ...
        <div class="form-group">
            <label for="amount" class="control-label"><i class="fas fa-money-check-alt"></i> Сумма к оплате клиентом: </label>
        ...
        <div class="form-group">
            <label for="" class="control-label"><i class="fas fa-exclamation-triangle"></i>  Состояние: </label>
            <select name="status" id="" class="custom-select custol-select-sm">
                <option value="0" <?php echo $status == 0 ? "selected" : '' ?>> В ожидании </option>
                <option value="1" <?php echo $status == 1 ? "selected" : '' ?>> Подтвержденный </option>
                <option value="2" <?php echo $status == 2 ? "selected" : '' ?>> Отменено </option>
                <option value="5" <?php echo $status == 3 ? "selected" : '' ?>> Подобрали </option>
                <option value="3" <?php echo $status == 4 ? "selected" : '' ?>> Возвращено </option>
            </select>
        </div>
    </form>
</div>
    ...
    $(function(){
        $('#date_start, #date_end').change(function(){
            ...
            $.ajax({
                ...
                error:err=>{
                    console.log(err)
                    alert_toast('Произошла ошибка при проверке доступности','error')
        ...
        $('#book-form').submit(function(e){
            ...
            if(_this.find('.border-danger').length > 0){
                alert_toast('Невозможно продолжить отправку из-за неверных данных в некоторых полях.','warning')
                return false;
            }
            $('.err-msg').remove();
            start_loader();
